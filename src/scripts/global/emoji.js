const Emoji = (() => {
  ...
  function emojisForMiniChat ($popup) {
    ...
    $popup.find('.open-emojis').on('click', () => {
    ...
    $(document).on('click', e => {
      const $target = $(e.target)
      if(!$target.closest($popup.find('.wrap-emojis')).length &&
        !$target.closest($popup.find('.open-emojis')).length &&
        $popup.find('.wrap-emojis').hasClass('is-active'))
      {
        $popup.find('.wrap-emojis').removeClass('is-active')
      }
    });
